...
bayes_multi = function(model, data, out, path, J, n){
  
  normalized = as.numeric(scale(data[[out]]))
  
  data_stan = list(N = nrow(data), y = normalized, treatment = as.numeric(data$treatment)-1, 
                   environment = as.numeric(data$env)-1, J = J, animal = rep(1:J, each = n))
  
  if (file.exists(path)) {fit = readRDS(path)} else {
    fit = mod$sample(data = data_stan,iter_warmup = 1000,iter_sampling = 2000,chains = 4,parallel_chains = 4, max_treedepth = 12,
                     output_dir = dirname(path))
    saveRDS(fit, path)}
